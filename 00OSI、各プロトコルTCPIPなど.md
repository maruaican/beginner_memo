...
*   **ネットワークインターフェース層**
    ...
    *   **主なプロトコル例**: Ethernet (第1-2層), Wi-Fi (第1-2層), PPP (第2層)
...
| 層の名称 | プロトコルの例 | 役割 |
| :--- | :--- | :--- |
| **アプリケーション層** | `HTTP, SMTP, FTP, …` | 利用者が直接触れるサービスを規定 |
| **セキュリティ層** | `TLS/SSL` | データの暗号化・認証を追加 |
| **トランスポート層** | `TCP / UDP` | 信頼性のある通信（TCP）などを提供 |
| **ネットワーク層** | `IP` | 宛先指定とルーティング |
| **データリンク層** | `Ethernet, Wi-Fi` | 同一ネットワーク内の物理的な伝送 |
| **物理層** | `ケーブル, 電波` | データを電気信号や電波に変換 |
