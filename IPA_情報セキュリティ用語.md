...
*   **RDP（Remote Desktop Protocol）**: Microsoft Windowsに標準で搭載されている、遠隔地のコンピュータをGUIで操作するための通信プロトコルです。非常に便利な機能である反面、認証さえ突破されれば内部のコンピュータを直接操作できるため、攻撃者にとっては格好の侵入口となります。
...
---
**【初心者が勘違いしやすい点】**
「リモートワイプがあるから、端末をなくしても安心だ」と考えるのは早計です。リモートワイプが成功するためには、端末の電源が入っており、かつインターネットに接続されている必要があります。紛失後すぐに電源を切られたり、SIMカードを抜かれたりした場合は、コマンドが届かずデータを消去できません。そのため、リモートワイプを過信せず、そもそも紛失しない、そしてディスク暗号化を有効にしておく、といった多層的な対策が重要です。
...
---
**【初心者が勘違いしやすい点】**
訓練でメールを開いてしまった従業員が、「怒られるのではないか」「評価が下がるのではないか」と恐れて、報告をためらってしまうことです。組織は、訓練の目的が個人の糾弾ではなく、組織防衛力の向上にあることを明確に伝え、「正直な報告が最善の貢献である」という文化（ノーブレイムカルチャー）を醸成する必要があります。失敗を責める組織では、インシデントは隠蔽され、発覚したときには手遅れになっているでしょう。

---
**【問41】**
内部不正の中でも、システム管理者などが持つ「特権ID」が悪用された場合の被害は特に甚大になります。なぜ特権IDの悪用は、一般ユーザーによる不正行為と比較して格段に危険なのでしょうか。そのIDを使って可能となる不正行為の例を2つ挙げ、このリスクに対する最も基本的な組織的対策の原則を述べてください。

---
**【解答】】**
【危険な理由】
特権IDは、システム内のほぼ全てのデータへのアクセスや設定変更が可能な「万能の鍵」だからです。一般ユーザーの権限が限定されているのに対し、特権IDはあらゆる制限を超えて操作できます。

【不正行為の例】
1.  個人情報や技術情報など、通常はアクセスできない機密情報を大量に窃取する。
2.  自らの不正行為を隠蔽するために、システムの監査ログやアクセスログを消去・改ざんする。

【対策の基本原則】
**「最小権限の原則」**。従業員には、その業務を遂行するために必要最小限の権限のみを付与し、不要な特権IDは与えないという原則を徹底することです。

---
**【解説】**
「神の権限」とも呼ばれる特権IDの管理は、内部不正対策の要です。対策としては、「最小権限の原則」を徹底した上で、やむを得ず特権IDを利用する際には、①利用申請と承認プロセスを厳格化する、②利用中は操作内容を全て記録（モニタリング）する、③利用後は速やかにパスワードを変更する、といった厳格な運用ルールを定めることが不可欠です。

---
**【初心者が勘違いしやすい点】**
「長年勤めている信頼できる管理者に任せているから、細かい管理は不要だ」という、性善説に基づいた思い込みが最も危険です。人は状況によって変化する可能性があり、また、その管理者自身が標的型攻撃を受けて特権IDを乗っ取られる可能性もあります。対策は、特定の個人への信頼に依存するのではなく、誰が使っても不正が起きない、あるいは不正が起きても必ず検知できる「仕組み」によって構築されるべきです。
