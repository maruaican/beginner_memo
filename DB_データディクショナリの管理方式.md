...
* 名前の由来：英語で「Data Dictionary / Directory」、直訳すると「データの辞書・目録」です。
  ...
  * 料理の「レシピ（調理手順書）」のたとえ
  * ただ「カレーライス」という料理名（テーブル名）があるだけでなく、「材料（カラム名）」として「じゃがいも」「人参」「牛肉」があり、それぞれ「個数（データ型：整数）」「グラム数（データ型：整数）」「産地（データ型：文字列、30文字以内）」といった詳細なルールや定義が書かれています。これがDD/Dの役割です。
  DD/Dがなければ、データベースはただのゴチャゴチャしたデータの集まりになってしまいます。どのデータが何を意味し、どんなルールに従うべきかを定義するDD/Dがあるからこそ、私たちは一貫性のある、信頼できるデータを維持管理できるのです。
