...
* **用語定義問題**：各用語の「主目的」「主対象」を即答できること（例：CASB＝クラウド可視化、EDR＝端末の挙動検知）。
* **比較問題**：類似技術を区別する問題が頻出（例：EDR vs AV／CASB vs SWG／SOAR vs SIEM）。解答時は「主な差分（何をする／しない）」をまず確認する。
* **適用場面提示問題**：ユースケースに最適な技術を選ばせる問題（例：誤設定で公開されたS3→CSPM）。
* **運用問題**：MFAやIDフェデレーション、プロキシ設定など運用上の注意点（ログ保存、ログの保全、監査証跡）を理解しているかを問う。
* **試験トリック**：

  * 「制御する対象（Webアクセス／クラウドアプリ／ファイル）」を見誤らない。
  * 「検出系（EDR/UEBA/SIEM）」と「予防系（IDaaS/IRM/SWG/CSPM）」を混同しない。
* **必要な知識**：プロトコル（SAML/OIDC/OAuth）、ログの種類（認証ログ／アクセスログ／操作ログ）、基本的なクラウド構成（VPC, IAM, ストレージ公開設定）を押さえる。
...
* 「ゼロトラスト」：英語“Zero Trust”はPerimeter（境界）信頼を前提とした古典的防御の限界を受け、「内部も信用しない」モデルとして2010年代にNIST等で整理された。
* （各用語の語源は各技術欄に1〜2文で記載済み）
