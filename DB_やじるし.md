...
* ID、番号、コード　→　日付、年月
* 部署ID　→　従業員ID　　　※「部署ごとに」との記載があれば、従業員が多いとわかるので、部署＜従業員となる。
...
* ❌ 「E-R図の１→多は、関数従属性の１→多と同じ意味」
  → **誤り**（前者はエンティティ間の件数関係、後者は同一表内の決定関係）
* ❌ 「外部キーがあるから必ず１対多」
...
**A1.** １つの部署に複数の従業員が所属する。
**解説**：E-R図では１→多の関係を表す。

**Q2.** 正規化における「社員ID → 社員名」の意味は？
